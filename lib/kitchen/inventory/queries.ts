...
export async function getKitchenInventoryOverviewStats(tenantId: string) {
  const [items, balances, movements, stockRules] = await Promise.all([
    ...
    (async () => {
      ...
      const { data, error } = await supabase
        ...
        .eq("tenant_id", tenantId)
        .eq("is_active", true);
      if (error) throw new Error(`No fue posible listar reglas de stock: ${error.message}`);
      return (data ?? []) as Array<{ item_id: string; location_id: string | null; min_quantity: number | null }>;
    })(),
  ]);
...
export async function listKitchenInventoryItemOperationalRows(
  ...
  const [items, balances, locations, purchaseOptions, supplierPrices] = await Promise.all([
  ...
  const defaultPoRowByItem = new Map<string, KitchenInventoryPurchaseOption>();
  for (const option of purchaseOptions) {
    if (option.is_active && option.is_default) {
      defaultPoByItem.set(option.item_id, true);
      if (!defaultPoRowByItem.has(option.item_id)) {
        defaultPoRowByItem.set(option.item_id, option);
      }
    }
  }
  ...
  for (const price of supplierPrices) {
    if (price.is_current) {
      ...
      if (!currentPriceRowByItem.has(price.item_id)) {
        currentPriceRowByItem.set(price.item_id, price);
      }
    }
  }
  ...
  return items.map((item) => {
    ...
    const defaultPurchaseOption = defaultPoRowByItem.get(item.id) ?? null;
...
export async function listPurchaseOptionsForItem(
  ...
  const options = await listPurchaseOptions(tenantId);
  return options.filter((row) => row.item_id === itemId);
}
...
export async function listSupplierPricesForItem(
  ...
  const rows = await listSupplierPrices(tenantId);
