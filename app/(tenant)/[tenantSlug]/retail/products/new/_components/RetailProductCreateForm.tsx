...
type RetailProductCreateFormProps = {
  action: (
...
export function RetailProductCreateForm({
  action,
  cancelHref,
  suppliers,
  tenantSlug,
  ...
  return (
    <form action={formAction} className="space-y-6 rounded-[var(--radius-base)] border border-border bg-surface p-4">
      <input type="hidden" name="tenantSlug" value={tenantSlug} />

      <section className="space-y-4">
        <div className="space-y-1">
          <h2 className="text-base font-semibold text-foreground">Informacion principal</h2>
          <p className="text-sm text-muted">Nuevo producto para el catalogo operativo POS retail.</p>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          <label className="space-y-1 text-sm">
            <span className="text-muted">Nombre</span>
            <input type="text" name="name" required defaultValue={values.name} className="w-full rounded-[var(--radius-base)] border border-border bg-surface-2 px-3 py-2" placeholder="Ej. Agua natural 1 L" />
            {renderFieldError(state.fieldErrors.name)}
          </label>
          <label className="space-y-1 text-sm">
            <span className="text-muted">Marca</span>
            <input type="text" name="brand" defaultValue={values.brand} className="w-full rounded-[var(--radius-base)] border border-border bg-surface-2 px-3 py-2" placeholder="Sin marca" />
      ...
      <section className="space-y-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted">SKU y barcode son opcionales; vacios se guardan como null.</p>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          <label className="space-y-1 text-sm">
            <span className="text-muted">SKU</span>
            <input type="text" name="sku" defaultValue={values.sku} className="w-full rounded-[var(--radius-base)] border border-border bg-surface-2 px-3 py-2" placeholder="Opcional" />
            {renderFieldError(state.fieldErrors.sku)}
          </label>
          <label className="space-y-1 text-sm">
            <span className="text-muted">Barcode</span>
            <input type="text" name="barcode" defaultValue={values.barcode} className="w-full rounded-[var(--radius-base)] border border-border bg-surface-2 px-3 py-2" placeholder="Opcional" />
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <label className="space-y-1 text-sm">
            ...
            <input type="number" name="cost" min="0" step="0.01" defaultValue={values.cost} className="w-full rounded-[var(--radius-base)] border border-border bg-surface-2 px-3 py-2" placeholder="Opcional" />
      ...
      <section className="space-y-4">
        ...
        <label className="block space-y-1 text-sm">
          ...
          <select name="supplier_id" defaultValue={values.supplier_id} className="w-full rounded-[var(--radius-base)] border border-border bg-surface-2 px-3 py-2">
      ...
      <section className="space-y-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted">Default real de las-quintas: `pza / Pieza / sin decimales`.</p>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          <label className="space-y-1 text-sm">
            <span className="text-muted">Codigo unidad</span>
            <input type="text" name="sales_unit_code" required defaultValue={values.sales_unit_code} className="w-full rounded-[var(--radius-base)] border border-border bg-surface-2 px-3 py-2" />
            {renderFieldError(state.fieldErrors.sales_unit_code)}
          </label>
          <label className="space-y-1 text-sm">
            <span className="text-muted">Etiqueta unidad</span>
            <input type="text" name="sales_unit_label" required defaultValue={values.sales_unit_label} className="w-full rounded-[var(--radius-base)] border border-border bg-surface-2 px-3 py-2" />
            {renderFieldError(state.fieldErrors.sales_unit_label)}
          </label>
        </div>
        <label className="inline-flex items-center gap-2 text-sm text-foreground">
          <input type="checkbox" name="allow_decimal_quantity" defaultChecked={values.allow_decimal_quantity} className="h-4 w-4 accent-primary" />
      ...
      <section className="space-y-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted">Se recomienda crear activo; tambien puede quedar inactivo sin borrado.</p>
        </div>
        <label className="inline-flex items-center gap-2 text-sm text-foreground">
          <input type="checkbox" name="is_active" defaultChecked={values.is_active} className="h-4 w-4 accent-primary" />
          Producto activo
        </label>
      </section>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Link href={cancelHref} className="inline-flex items-center justify-center rounded-[var(--radius-base)] border border-border bg-surface-2 px-4 py-2 text-sm font-medium text-foreground transition-opacity duration-200 hover:opacity-90">
          Cancelar
        </Link>
      </div>
    </form>
