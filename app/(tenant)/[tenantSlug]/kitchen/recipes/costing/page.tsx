...
export default async function KitchenRecipesCostingPage({ params }: KitchenRecipesCostingPageProps) {
  ...
  return (
    <div className="space-y-4">
      <KitchenPageHeader
        eyebrow="Costeo"
        title="Tablero de costeo"
        description="Costo base de receta calculado desde inventario, con problemas de costeo por faltantes de costo o conversiones."
      />
...
async function RecipeCostTableSection({
  uiTenantSlug,
  tenantId,
}: {
  uiTenantSlug: string;
  ...
  const [recipes, readiness] = await Promise.all([
    listKitchenRecipes(tenantId),
    listKitchenRecipeReadiness(tenantId),
  ]);
        ...
        <table className="w-full min-w-[920px] text-left text-sm">
          ...
          <tbody>
            {recipeCosts.map(({ recipe, version, cost }) => {
              ...
              return (
                <tr key={recipe.id} className="border-t border-border transition-colors hover:bg-surface-2/50">
                  <td className="py-2">
                    <Link href={`/${uiTenantSlug}/kitchen/recipes/${recipe.id}`} className="underline underline-offset-2">
                      {recipe.name}
                    </Link>
                  </td>
                  <td className="py-2">
          ...
          <table className="w-full min-w-[860px] text-left text-sm">
            ...
            <tbody>
              {snapshots.map((snapshot) => (
                <tr key={snapshot.id} className="border-t border-border transition-colors hover:bg-surface-2/50">
