...
export default async function KitchenEventCateringPlanPage({ params }: KitchenEventCateringPlanPageProps) {
  ...
  const [plan, planRecipes, readyRecipes, requirements, shortages, summary, draftRequisition, operationalSummary, itemFlow, warnings, accessMap] = await Promise.all([
    getCateringPlan(result.tenant.tenantSlug, planId),
    listPlanRecipes(result.tenant.tenantSlug, planId),
    listReadyRecipesForCatering(result.tenant.tenantSlug),
    listCateringRequirements(result.tenant.tenantSlug, planId),
    listRequirementShortages(result.tenant.tenantSlug, planId),
    getCateringPlanSummary(result.tenant.tenantSlug, planId),
    getPlanDraftRequisition(result.tenant.tenantSlug, planId),
    getCateringPlanOperationalSummary(result.tenant.tenantSlug, planId),
    listCateringPlanItemFlow(result.tenant.tenantSlug, planId),
    listCateringPlanWarnings(result.tenant.tenantSlug, planId),
  ...
  const event = await getEventForCatering(result.tenant.tenantSlug, eventId);
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-[var(--radius-base)] border border-border bg-surface p-4">
        <h1 className="text-lg font-semibold text-foreground">{plan.name ?? `Plan ${plan.id.slice(0, 8)}`}</h1>
        <p className="mt-2 text-sm text-muted">
          event={eventId} · status={plan.status} · costo estimado=${Number(plan.estimated_total_cost).toLocaleString("es-MX", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
        </p>
        <p className="mt-1 text-xs text-muted">
          Asistencia esperada evento={expectedAttendance > 0 ? expectedAttendance.toLocaleString("es-MX") : "—"} ·
          base del plan={plannedGuestCount > 0 ? plannedGuestCount.toLocaleString("es-MX") : "—"}
          {hasGuestDelta
            ? ` · diferencia=${guestDelta > 0 ? "+" : ""}${guestDelta.toLocaleString("es-MX")}`
            : ""}
        </p>
        <p className="mt-1 text-xs text-muted">
          El plan puede diferir del evento si este servicio aplica solo a parte de los asistentes.
        </p>
        <p className="mt-1 text-xs text-muted">
          requerimientos={summary.totalRequiredLines} · faltantes={summary.shortageCount} · costo faltante=${Number(summary.totalShortageCost).toLocaleString("es-MX", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
        </p>
        <div className="mt-2">
          <Link
      ...
      <section className="rounded-[var(--radius-base)] border border-border bg-surface p-4">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded border border-border bg-surface-2 p-2 text-xs text-muted">Costo estimado<br /><span className="text-foreground">${Number(operationalSummary.estimated_plan_cost).toLocaleString("es-MX",{minimumFractionDigits:2,maximumFractionDigits:2})}</span></div>
          <div className="rounded border border-border bg-surface-2 p-2 text-xs text-muted">Requisicionado<br /><span className="text-foreground">${Number(operationalSummary.requisition_total).toLocaleString("es-MX",{minimumFractionDigits:2,maximumFractionDigits:2})}</span></div>
          <div className="rounded border border-border bg-surface-2 p-2 text-xs text-muted">Recibido<br /><span className="text-foreground">${Number(operationalSummary.received_total_cost).toLocaleString("es-MX",{minimumFractionDigits:2,maximumFractionDigits:2})}</span></div>
          <div className="rounded border border-border bg-surface-2 p-2 text-xs text-muted">Consumido<br /><span className="text-foreground">${Number(operationalSummary.consumed_total_cost).toLocaleString("es-MX",{minimumFractionDigits:2,maximumFractionDigits:2})}</span></div>
          <div className="rounded border border-border bg-surface-2 p-2 text-xs text-muted">Merma<br /><span className="text-foreground">${Number(operationalSummary.waste_total_cost).toLocaleString("es-MX",{minimumFractionDigits:2,maximumFractionDigits:2})}</span></div>
          <div className="rounded border border-border bg-surface-2 p-2 text-xs text-muted">Faltantes<br /><span className="text-foreground">{operationalSummary.shortage_count}</span></div>
          <div className="rounded border border-border bg-surface-2 p-2 text-xs text-muted">Var. recibido vs requerido<br /><span className="text-foreground">{Number(operationalSummary.variance_received_vs_required).toLocaleString("es-MX",{minimumFractionDigits:2,maximumFractionDigits:4})}</span></div>
          <div className="rounded border border-border bg-surface-2 p-2 text-xs text-muted">Var. consumido vs recibido<br /><span className="text-foreground">{Number(operationalSummary.variance_consumed_vs_received).toLocaleString("es-MX",{minimumFractionDigits:2,maximumFractionDigits:4})}</span></div>
        </div>
      </section>
            ...
            {planRecipes.map((planRecipe) => (
              <div key={planRecipe.id} className="rounded border border-border bg-surface-2 p-3">
                <p className="text-sm text-foreground">
                  {planRecipe.kitchen_recipe_recipes?.name ?? `recipe=${planRecipe.recipe_id.slice(0, 8)}`} ·
                ...
                <p className="text-xs text-muted">
                  ...
                  multiplier={Number(planRecipe.multiplier).toLocaleString("es-MX", { minimumFractionDigits: 2, maximumFractionDigits: 4 })} ·
                  costo base=${Number(planRecipe.estimated_cost).toLocaleString("es-MX", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ·
                  estado receta={planRecipe.kitchen_recipe_recipes?.status ?? "—"}
                </p>
                {canManagePlans ? (
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    <form action={updatePlanRecipeServingsAction} className="flex items-center gap-2">
                      ...
                      />
                      <button type="submit" className="inline-flex rounded-[var(--radius-base)] border border-border bg-surface px-2 py-1 text-xs">
                        Actualizar base de cálculo
                      </button>
                    </form>
                    <form action={removeRecipeFromCateringPlanAction}>
                      ...
                      <input type="hidden" name="planId" value={plan.id} />
                      <input type="hidden" name="planRecipeId" value={planRecipe.id} />
                      <button type="submit" className="inline-flex rounded-[var(--radius-base)] border border-border bg-surface px-2 py-1 text-xs">Quitar receta</button>
                    </form>
                  </div>
                ) : null}
              </div>
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          {canManageRequirements ? (
            <form action={recalculateCateringRequirementsAction}>
              <input type="hidden" name="tenantSlug" value={tenantSlug} />
              <input type="hidden" name="planId" value={plan.id} />
              <button type="submit" className="inline-flex rounded-[var(--radius-base)] border border-border bg-surface-2 px-3 py-2 text-xs">
          ...
          <div className="mt-3 overflow-x-auto">
            ...
            {shortages.length > 0 ? (
              <div className="mt-2 flex flex-wrap items-center gap-2">
                ...
                {draftRequisition ? (
                  <Link
                    href={`/${tenantSlug}/kitchen/events/requisitions/${draftRequisition.id}`}
