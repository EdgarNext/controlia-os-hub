...
export default async function KitchenEventsCorrectionsPage({ params }: KitchenEventsCorrectionsPageProps) {
  ...
  const reversals = await listInventoryReversals(result.tenant.tenantSlug);
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-[var(--radius-base)] border border-border bg-surface p-4">
        <h1 className="text-lg font-semibold text-foreground">Correcciones de inventario</h1>
        <p className="mt-1 text-xs text-muted">
          Las correcciones no borran movimientos históricos. Una corrección aplicada crea movimientos compensatorios y conserva trazabilidad completa.
        </p>
      </section>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {row.status === "draft" ? (
                          <form action={cancelInventoryReversalDraftAction}>
                            ...
                            <button type="submit" className="rounded border border-border bg-surface px-2 py-1 text-xs">
                              Cancelar
                            </button>
                          </form>
                        ) : null}
