...
export default async function KitchenInventoryMovementsPage({
  ...
  const [movements, items, locations, units, accessMap] = await Promise.all([
    listKitchenInventoryRecentMovements(result.tenant.tenantId, 80),
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-[var(--radius-base)] border border-border bg-surface p-4">
        <h1 className="text-lg font-semibold text-foreground">Movimientos</h1>
        <p className="mt-2 text-sm text-muted">
          Ledger operacional de entradas, salidas, ajustes y mermas con bloqueo de inventario negativo por default.
        </p>
      </section>
