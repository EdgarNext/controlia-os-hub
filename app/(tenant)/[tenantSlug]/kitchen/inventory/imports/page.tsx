...
export default async function KitchenInventoryImportsPage({ params }: KitchenInventoryImportsPageProps) {
  ...
  const [batches, accessMap] = await Promise.all([
    listKitchenInventoryImportBatches(result.tenant.tenantId),
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-[var(--radius-base)] border border-border bg-surface p-4">
        <h1 className="text-lg font-semibold text-foreground">Importaciones de inventario</h1>
        <p className="mt-2 text-sm text-muted">
          Flujo controlado: parsear Excel a staging, validar filas y aplicar con idempotencia.
        </p>
      </section>
