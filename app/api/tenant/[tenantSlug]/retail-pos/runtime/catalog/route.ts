...
export async function GET(request: NextRequest, context: { params: Promise<RouteParams> }) {
  ...
  try {
    ...
    const q = getOptionalSearchParam(request, "q");
    const sku = getOptionalSearchParam(request, "sku");
    const barcode = getOptionalSearchParam(request, "barcode");
    ...
    const message = error instanceof Error ? error.message : "Unexpected retail_pos catalog runtime error.";
