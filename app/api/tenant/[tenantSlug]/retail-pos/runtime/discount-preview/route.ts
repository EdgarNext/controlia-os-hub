...
import { previewRetailPosOrderDiscounts } from "@/lib/retail-pos/discount-preview";
...
export async function POST(
  ...
  const trace = createRuntimePerfTrace({
    request,
    route: "retail-pos/discount-preview",
    method: "POST",
    tenantSlug,
  ...
  try {
    ...
    const payload = await trace.measure("business", () =>
      previewRetailPosOrderDiscounts({
        tenantSlug,
        payload:
          body as Parameters<typeof previewRetailPosOrderDiscounts>[0]["payload"],
        deviceId:
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Unexpected retail_pos discount preview error.";
    const response = jsonError({
      status: 500,
      message,
      requestId: trace.requestId,
