...
import { confirmRetailPosCardRefund } from "@/lib/retail-pos/post-sale";
...
export async function POST(
  ...
  const trace = createRuntimePerfTrace({
    request,
    route: "retail-pos/post-sale/refunds/card/confirm",
    method: "POST",
    tenantSlug,
  ...
  try {
    const body = (await request.json()) as ConfirmBody | null;
    ...
    const payload = await trace.measure("business", () =>
      confirmRetailPosCardRefund({
        tenantSlug,
        commandId: body.command_id,
        operatorId: body.operator_id,
        request: {
          post_sale_document_id: body.post_sale_document_id,
    ...
    const response = jsonError({
      ...
      message:
        error instanceof Error
          ? error.message
          : "Unexpected retail_pos card refund confirmation error.",
      requestId: trace.requestId,
    });
    Object.entries(trace.headers()).forEach(([key, value]) =>
      response.headers.set(key, value),
