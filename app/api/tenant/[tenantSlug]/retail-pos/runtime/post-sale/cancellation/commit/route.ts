...
import { commitRetailPosSaleCancellation } from "@/lib/retail-pos/post-sale";
...
export async function POST(
  ...
  const trace = createRuntimePerfTrace({
    request,
    route: "retail-pos/post-sale/cancellation/commit",
    method: "POST",
    tenantSlug,
  ...
  try {
    ...
    const payload = await trace.measure("business", () =>
      commitRetailPosSaleCancellation({
        tenantSlug,
        ...
        request: {
          ...
          reason_code: body.reason_code as Parameters<
            typeof commitRetailPosSaleCancellation
    ...
    const response = jsonError({
      ...
      message:
        error instanceof Error
          ? error.message
          : "Unexpected retail_pos post sale cancellation commit error.",
      requestId: trace.requestId,
    });
    Object.entries(trace.headers()).forEach(([key, value]) =>
      response.headers.set(key, value),
