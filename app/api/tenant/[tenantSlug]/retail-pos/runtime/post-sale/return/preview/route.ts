...
import { previewRetailPosReturn } from "@/lib/retail-pos/post-sale";
...
export async function POST(
  ...
  const trace = createRuntimePerfTrace({
    request,
    route: "retail-pos/post-sale/return/preview",
    method: "POST",
    tenantSlug,
  ...
  try {
    ...
    const response = jsonError({
      status: 500,
      message:
        error instanceof Error ? error.message : "Unexpected retail_pos post sale return preview error.",
      requestId: trace.requestId,
    });
    Object.entries(trace.headers()).forEach(([key, value]) => response.headers.set(key, value));
